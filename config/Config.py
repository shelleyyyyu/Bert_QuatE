...
class Config(object):
    ...
    def set_train_model(self, model):
        ...
        if torch.cuda.is_available():
            #self.trainModel = nn.DataParallel(self.trainModel)
    ...
    def set_test_model(self, best_model, path=None):
        print("Initializing test model...", flush=True)
        self.testModel = self.trainModel
        self.testModel.load_state_dict(best_model)
        if torch.cuda.is_available():
            self.testModel.cuda()
        self.testModel.eval()
    ...
    def save_checkpoint(self, model, epoch):
        path = os.path.join(
            self.checkpoint_dir, self.model.__name__ + "-" + ('%04d' % epoch) + ".ckpt"
        )
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.train_times)):
            ...
            for batch in range(self.nbatches):
                ...
                res += loss
            print("Epoch %04d | loss: %8.5f" % (epoch, res), flush=True)

            if (epoch + 1) % self.valid_steps == 0:
                print("Epoch %04d has finished, validating..." % (epoch), flush=True)
                hit10 = self.valid(self.trainModel)
                if hit10 > best_hit10:
                    best_hit10 = hit10
                    best_epoch = epoch
                    best_model = self.trainModel.state_dict()
                    print("Best model | hit@10 of valid set is %5.3f" % (best_hit10), flush=True)
                    bad_counts = 0
                else:
                    print(
                        "Hit@10 of valid set is %5.3f | bad count is %03d"
                        % (hit10, bad_counts), flush=True
                    )
                    bad_counts += 1
                if bad_counts == self.early_stopping_patience:
                    print("Early stopping at epoch %04d" % (epoch), flush=True)
                    break
        if best_model == None:
            best_model = self.trainModel.state_dict()
            best_epoch = self.train_times - 1
            best_hit10 = self.valid(self.trainModel)
        print("Best epoch is %04d | hit@10 of valid set is %5.3f" % (best_epoch, best_hit10), flush=True)

        print("Finish storing", flush=True)
        print("Testing...", flush=True)
        self.set_test_model(best_model)
        self.test()
        print("Finish test", flush=True)
        return best_model

    def link_prediction(self):
        print("The total of test triple is %d" % (self.testTotal), flush=True)
